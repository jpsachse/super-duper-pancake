 ...
 *
 * Modified by Jan Philipp Sachse.
 * Added triple slash comments to mark locations where a comment should be recognized as
 * low quality or where additional comments should be placed.
 */
...
export function loadConfigurationFromPath(configFilePath?: string, originalFilePath = configFilePath) {
    if (configFilePath == null) {
        ...
        if (path.extname(resolvedConfigFilePath) === ".json") {
            /// missing explanation
            /// There should be a comment here explaining this block.
            /// E.g., why we need to call toString and remove ZERO WIDTH NO-BREAK SPACE characters.
